...
t2_data_path='../data/t2_data_sim.mat';%Path to the mat-file containing T2 data
op_folder='';%Directory where the outputs will be saved
op_filename='stats_table_r2_fit_results';%A csv file that contains parameter estimates from different fitting routines. Can be read in R, xl or any other spread-sheet software.
...
header={'vox','algo','r2','foffset','ph','wf','ntf','snr','param','evalue','tvalue'};% The header of the csv file
% vox: voxel number, algo: Algorithm, r2: True R2* value,
...
r2_fit_results=cell([length(fit_routines) szdata([2 4:6]) length(weight_flag) length(noise_thresh_flag)]);% Create a cell to hold all the results (in case we need to save it to a matfile)
outliers=r2_fit_results;% Create a cell to hold all outliers as well
    ...
    for snr_idx=1:szdata(6)% For each selected SNR
        for f_idx=1:szdata(4)% For each frequency deviation condition
            for ph_idx=1:szdata(5)% For every starting phase condition
                for r2_idx=1:szdata(2)% For every simulated T2* (R2*)
                    for noise_thresh_idx=1:length(noise_thresh_flag) % For different noise-thresholding conditions
                        for weight_idx=1:length(weight_flag)% For different weighting conditions
                            ...
                            disp_str4=['T2 (ms): ' num2str(1e3*T2(r2_idx))];
                            disp_str5=['R2 (1/s): ' num2str(1/T2(r2_idx))];
                            ...
                            temp_data=squeeze(t2_data(:,r2_idx,:,f_idx,ph_idx,snr_idx)); % Select the appropriate data
                            ...
                            temp_outliers=generate_t2s_stats_table_outliers_removed(all_algos,fp,temp_results,snrs(snr_idx),f_range(f_idx),start_ph(ph_idx),T2(r2_idx),fit_routines(algo_idx),weight_flag(weight_idx),noise_thresh_flag(noise_thresh_idx));
                            disp(' ');%For pretty printing
                            
                            r2_fit_results{algo_idx,r2_idx,f_idx,ph_idx,snr_idx,weight_idx,noise_thresh_idx}=temp_results;% Save the results in a cell
                            outliers{algo_idx,r2_idx,f_idx,ph_idx,snr_idx,weight_idx,noise_thresh_idx}=temp_outliers;% Save the outliers in a cell
                        end
                    end
                end
            end
...
save([op_folder 'r2_fit_results.mat'],'r2_fit_results');
save([op_folder 'r2_outlier_results.mat'],'outliers');
