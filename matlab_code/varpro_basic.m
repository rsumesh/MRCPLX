function [alpha, c, wresid, wresid_norm2, exitflag] = ...
    varpro_basic(y, w, alpha, n, ada, lb, ub, options)

%VARPRO_BASIC: A stripped-down version of VARPRO, so that the oveheads are
%minimized for repeated calculations and to estimate computational burdern.
%Based on VARPRO, details related to which are below.
% Modified by: Suryanarayana Umesh Rudrapatna
% e-mail: umeshrs at gmail.com
% Release date: Aug 29, 2015
% Note: Matlab users with Curve-fitting toolbox can also use lsqcurvefit to
% obtain similar functionality.
...
[m,~] = size(y);      % m = number of observations
% [m1,ell1] = size(w);
...
[q,~] = size(alpha);   % q = number of nonlinear parameters
...
[Phi, ~, Ind] = feval(ada, alpha);

[~,n1] = size(Phi);     % n1 = number of basis functions Phi.
...
[alpha, wresid_norm2, wresid, exitflag] = ...
