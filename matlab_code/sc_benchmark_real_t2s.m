...
load('../data/t2s_benchmarking_data.mat');%Load benchmarking data (1000 example time series from SNR-degraded real data)
...
exec_times=zeros(size(t2s_data,1),length(fit_routines));
t2s_est=exec_times;

warning off
for idx=1:size(t2s_data,1)
    disp(num2str(idx));
    for routine_idx=1:length(fit_routines)
        % Function that was used to obtain the benchmarking results
        [temp_exec_time,temp_t2s_est]=benchmark_t2_exec_time(t2s_data(idx,:),te,fit_routines(routine_idx));
...
save('t2s_benchmarking_results.mat','exec_times','t2s_est');