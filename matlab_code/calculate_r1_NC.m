function [r1_fit_results,tTimeit]=calculate_r1_NC(data,ti,r1_lb,r1_ub,weight_flag,noise_thresh_flag,noise_thresh,varargin)
...
for idx=1:length(phenc_pos)
    ...
    if fit_numvals>2
        ...
        if idx==1 %benchmarking
            f=@()fminspleas({@(params,ti)(1-(params(2)*exp(-ti(:)*(params(2)-1)*params(1))))},NLPstart,ti,time_series,INLB,INUB,w,options);
            tTimeit=timeit(f);
        end
        
        [INLP,ILP,eflag,fval]=fminspleas({@(params,ti)(1-(params(2)*exp(-ti(:)*(params(2)-1)*params(1))))},NLPstart,ti,time_series,INLB,INUB,w,options);
        predicted=ILP*(1-(INLP(2)*exp(-ti(:)*(INLP(2)-1)*INLP(1))));
        
        error=norm(time_series-predicted)/norm(time_series);
        r1_fit_results(rdout_pos(idx),phenc_pos(idx),:)=[INLP(:);angle(ILP);abs(ILP);error;eflag;fval;fit_numvals];
    else
