function [INLP,ILP,eflag,fval] = fminspleas_rician(funlist,NLPstart,xdata,ydata,INLB,INUB,weights,noise_var,options)
% pleas: partitioned nonlinear least squares estimation, using fminsearch
% assuming Rician noise distribution
% usage: [INLP,ILP,eflag,fval] = FMINSPLEAS(funlist,NLPstart,xdata,ydata)
% usage: [INLP,ILP,eflag,fval] = FMINSPLEAS(funlist,NLPstart,xdata,ydata,INLB,INUB)
% usage: [INLP,ILP,eflag,fval] = FMINSPLEAS(funlist,NLPstart,xdata,ydata,INLB,INUB,weights)
% usage: [INLP,ILP,eflag,fval] = FMINSPLEAS(funlist,NLPstart,xdata,ydata,INLB,INUB,weights,noise_var,options)
% A modification of FMINSPLEAS to accommodate Rician noise assumption
...
fminunc_options=optimset('fminunc');
fminunc_options=optimset(fminunc_options,'Algorithm','interior-point','LargeScale','off');
fminunc_options.Display='off';
fminunc_options.TolFun=1e-3;
fminunc_options.TolX=1e-3;
...
%   
%   predicted_vals=A*ILP;
  
%   warning on
  % sum of squares objective for fminsearch. Weights are
  % built in already here.
  
  % We need to find an A such that the objective is minimized
   [opt_A,obj,eflag]=fminunc(@(x)obj_rician(x,ydata,A,noise_var,weights),ILP,fminunc_options);% This is the modification to regular fminspleas
...
[tINLP,fval,eflag] = fminsearch(@fminspleas_obj,tNLPstart,options);
