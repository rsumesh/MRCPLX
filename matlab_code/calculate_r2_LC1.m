function [r2_fit_results,tTimeit,fit_errors]=calculate_r2_LC1(data,te,t2_upper_threshold,max_f_deviation,weight_flag,noise_thresh_flag,noise_thresh,varargin)

% Function to calculate R2 maps using a derivative-free regular least-squares method on complex-valued data
...
start_vals=[1;0;100e-3;0];
LB=[-5;-5;0;-max_f_deviation];
UB=[5;5;t2_upper_threshold;max_f_deviation];
tTimeit=0;
...
for idx=1:length(phenc_pos)
    ...
    if fit_numvals>3
        ...
        if idx==1 %benchmarking
            f=@()fminsearchbnd(@(params)r2_LC1(params,te,time_series,w),start_vals,LB,UB,options);
            tTimeit=timeit(f);
        end
        
        [opt_params,fval,eflag]=fminsearchbnd(@(params)r2_LC1(params,te,time_series,w),start_vals,LB,UB,options);
        ...
        r2_fit_results(rdout_pos(idx),phenc_pos(idx),:)=[(1/opt_params(3));angle(ILP);opt_params(4);abs(ILP);error;eflag;abs(fval);fit_numvals];
    else
...
%% Function r2_LC1
    function obj=r2_LC1(params,te,time_series,w)
        
        magnetization=(params(1)+1i*params(2))*exp(-te(:)/params(3)).*exp(1i*2*pi*params(4)*te(:));
        obj=norm(w(:).*(magnetization-time_series));
    end
