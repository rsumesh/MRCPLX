function [r1_fit_results,tTimeit]=calculate_r1_LC2(data,ti,r1_lb,r1_ub,weight_flag,noise_thresh_flag,noise_thresh,varargin)

% Function to calculate R2 maps using a derivative-based method (the same in VARPRO) on complex-valued data
...
for idx=1:length(phenc_pos)
    ...
    if fit_numvals>2
        ...
        if idx==1 %benchmarking
            f=@()r1_LC2(time_series,w,ti,startvals,LB,UB,options);
            tTimeit=timeit(f);
        end
        
        [estimates,fval,eflag]=r1_LC2(time_series,w,ti,startvals,LB,UB,options);
...
%% Function r1_LC2
    function [estimates,fval,eflag]=r1_LC2(data,w,ti,startvals,LB,UB,options)
        ...
        [estimates,fval,~,eflag]=lsqnonlin(@(params)obj_r1_LC2(params,ti,ydata,w),startvals,LB,UB,options);
    end
%% Function obj_r1_LC2
    function [obj,Jmtx]=obj_r1_LC2(params,ti,ydata,w)
        R1=params(3);
        B=params(4);
        ...
        Jmtx(:,1)=Phi;
        Jmtx(:,2)=1i*Phi;
        Jmtx(:,3)=amp*(Phi -1).*(-ti(:)*(B-1));
        Jmtx(:,4)=amp*exp_part.*(B*ti(:)*R1 -1);
