function [r1_fit_results,tTimeit]=calculate_r1_RM(data,ti,r1_lb,r1_ub,weight_flag,noise_thresh_flag,noise_thresh,noise_std,varargin)

% Function to calculate R1 maps using NIPALS on magnitude data but with Rician noise assumption
...
for idx=1:length(phenc_pos)
    ...
    if fit_numvals>2
        ...
        if idx==1 %benchmarking
            f=@()fminspleas_rician({@(params,ti)abs(1-(params(2)*exp(-ti(:)*(params(2)-1)*params(1))))},NLPstart,ti(:),time_series,INLB,INUB,w,noise_var,options);
            tTimeit=timeit(f);
        end
        
        [INLP,ILP,eflag,fval]=fminspleas_rician({@(params,ti)abs(1-(params(2)*exp(-ti(:)*(params(2)-1)*params(1))))},NLPstart,ti(:),time_series,INLB,INUB,w,noise_var,options);
